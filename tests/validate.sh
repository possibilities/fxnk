...
grep -F 'Workshop owns no second Full CI polling daemon' MAINTAIN.md >/dev/null \
    || fail "the gate does not retire the redundant Full CI watcher"
for retired_watch_path in \
    scripts/ci-watch.sh \
    scripts/ci-watch-install.sh \
    launchd/fxnk.ci-watch.plist \
    tests/ci-watch; do
    [ ! -e "$retired_watch_path" ] \
        || fail "retired Full CI watcher path remains: $retired_watch_path"
done
